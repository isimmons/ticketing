import request from 'supertest';
import mongoose from 'mongoose';
import { app } from '../../app';
import { natsWrapper } from '../../nats-wrapper';
import { signup } from './helpers/signup-helper';
import { Ticket } from '../../models/ticket';

const createTicket = (title: string, price: number, cookie?: string[]) => {
  let thisCookie;
  if (!cookie) thisCookie = signup();
  else thisCookie = cookie;

  return request(app)
    .post('/api/tickets')
    .set('Cookie', thisCookie)
    .send({
      title,
      price,
    })
    .expect(201);
};

it('returns a 404 if the provided ticket id does not exist', async () => {
  const id = new mongoose.Types.ObjectId().toHexString();

  await request(app)
    .put(`/api/tickets/${id}`)
    .set('Cookie', signup())
    .send({
      title: 'foobar',
      price: 20,
    })
    .expect(404);
});

it('returns a 401 if the user is not authenticated', async () => {
  const id = new mongoose.Types.ObjectId().toHexString();

  await request(app)
    .put(`/api/tickets/${id}`)
    .send({
      title: 'foobar',
      price: 20,
    })
    .expect(401);
});

it('returns a 401 if the user does not own the ticket', async () => {
  const response = await createTicket('foobar', 20);

  await request(app)
    .put(`/api/tickets/${response.body.id}`)
    .set('Cookie', signup())
    .send({
      title: 'barfoo',
      price: 10,
    })
    .expect(401);
});

it('returns a 400 if the user provides invalid title or price', async () => {
  const cookie = signup();

  const response = await createTicket('foo', 10, cookie);

  await request(app)
    .put(`/api/tickets/${response.body.id}`)
    .set('Cookie', cookie)
    .send({
      title: '',
      price: 20,
    })
    .expect(400);

  await request(app)
    .put(`/api/tickets/${response.body.id}`)
    .set('Cookie', cookie)
    .send({
      title: 'foobar',
      price: -20,
    })
    .expect(400);
});
it('updates the ticket when provided valid inputs', async () => {
  const cookie = signup();

  const response = await createTicket('foo', 10, cookie);

  await request(app)
    .put(`/api/tickets/${response.body.id}`)
    .set('Cookie', cookie)
    .send({
      title: 'foobarbaz',
      price: 50,
    })
    .expect(200);

  const ticketResponse = await request(app)
    .get(`/api/tickets/${response.body.id}`)
    .send()
    .expect(200);

  expect(ticketResponse.body.title).toEqual('foobarbaz');
  expect(ticketResponse.body.price).toEqual(50);
});

it('Publishes an event', async () => {
  const cookie = signup();

  const response = await createTicket('foo', 10, cookie);

  await request(app)
    .put(`/api/tickets/${response.body.id}`)
    .set('Cookie', cookie)
    .send({
      title: 'foobarbaz',
      price: 50,
    })
    .expect(200);

  expect(natsWrapper.client.publish).toHaveBeenCalled();
});

it('rejects the update if the ticket is reserved', async () => {
  const cookie = signup();

  const response = await createTicket('foo', 10, cookie);

  const ticket = await Ticket.findById(response.body.id);

  ticket!.set({ orderId: new mongoose.Types.ObjectId().toHexString() });
  await ticket!.save();

  await request(app)
    .put(`/api/tickets/${response.body.id}`)
    .set('Cookie', cookie)
    .send({
      title: 'foobarbaz',
      price: 50,
    })
    .expect(400);
});
